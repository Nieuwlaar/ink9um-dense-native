...
(or <z>.<y>.<x> when the remote array uses dimension_separator "." — pherc1667-w013).
Missing remote chunks (404) are fill-value chunks and are skipped. Idempotent:
existing full-size chunk files are not re-downloaded (compressor is null for all but
pherc1667-w013, whose blosc chunks are accepted by non-empty size).
"""
import json
import math
import sys
...
def main():
    ...
    compressed = meta["compressor"] is not None   # pherc1667-w013 level 2 is blosc-compressed
    sep = meta.get("dimension_separator", ".")     # w013 also uses "." keys (z.y.x); the rest "/"
    ...
    def fetch(key):
        z, y, x = key
        rel = f"{z}/{y}/{x}" if sep == "/" else f"{z}.{y}.{x}"
        out = dest / rel
        if out.exists() and (out.stat().st_size == chunk_bytes or (compressed and out.stat().st_size > 0)):
            return 0
        body = http_get(f"{base}/2/{rel}")
        if body is None:
            return 0  # fill-value chunk, not stored
        assert compressed or len(body) == chunk_bytes, f"bad chunk size {len(body)} for {key}"
